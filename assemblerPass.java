...
public class assemblerPass  {
	static String temp="";
	static int i,j,relAddr=0, count=0;
	static String[] keywords;
	static int m=0,p=0,s=0,l=0;
	...
	BufferedWriter ltFile = new BufferedWriter(new FileWriter("lt.txt",true));
		...
		while((temp = br.readLine())!=null) {
			...
			for(i=0;i<keywords.length;i++) {
				if(isMachine(keywords[i]))
					printMachineOp(keywords[i],motFile);
				else if(isPseudo(keywords[i]))
					printPseudoOp(keywords[i],potFile);
				else if(isSymbol(keywords[i]))
					printSymbol(keywords[i],relAddr,stFile);
				else if(isLiteral(keywords[i]))
					printLiteral(keywords[i],relAddr,ltFile);
			}
...
/*
C:\Users\Niti123\Desktop\coding\spcc>javac assemblerPass.java

C:\Users\Niti123\Desktop\coding\spcc>java assemblerPass
MOT TABLE
